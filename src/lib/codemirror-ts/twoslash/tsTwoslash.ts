 ...
 */
const setTwoSlashes = StateEffect.define({
    map: (sets, change) => {
        return sets.map((set) => {
            return {
 ...
 * the new decorations.
 */
function twoslashes(view, config) {
    if (!config)
        return null;
    const promises = [];
 ...
 */
const twoslashField = StateField.define({
    create() {
 ...
 */
const twoslashPlugin = ViewPlugin.fromClass(class {
    constructor(view) {
...
//# sourceMappingURL=tsTwoslash.map